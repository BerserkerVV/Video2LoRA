    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            for example in examples:
                ...
                # ref_values: 对 ref_values 也进行 Resize 和 CenterCrop 处理，确保尺寸足够大
                ref_pixel_values = torch.from_numpy(example["ref_values"]).permute(0, 3, 1, 2).contiguous()
                ref_pixel_values = ref_pixel_values / 255.
                
                # 使用与 pixel_values 相同的 transform 处理 ref_values
                ref_transformed = transform(ref_pixel_values)
                # 转换回 (F, H, W, 3) 格式
                ref_transformed = ref_transformed.permute(0, 2, 3, 1).contiguous()
                # 转换回 0-255 范围
                ref_transformed = (ref_transformed * 255.).numpy()
                new_examples["ref_values"].append(ref_transformed)
            ...
            # ref_values: 不裁剪帧数，保留完整视频供 hypernetwork 生成 weight
            new_examples["ref_values"] = torch.stack([torch.from_numpy(example) for example in new_examples["ref_values"]])
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if "ref_values" in batch:
                    ...
                    # 检查 ref_pixel_values 的尺寸，避免 VAE tiling 时尺寸太小导致下采样坍缩
                    # VAE tiling 要求最小尺寸至少为 64x64，否则 tile 会小于卷积核大小
                    _, _, _, ref_h, ref_w = ref_pixel_values.shape
                    print(f"[Debug] ref_pixel_values shape: {ref_pixel_values.shape}, ref_h={ref_h}, ref_w={ref_w}")
                    if ref_h >= 64 and ref_w >= 64:
                        print(f"[Debug] ref_values size OK ({ref_h}x{ref_w}), proceeding with hypernetwork update")
                        ...
                        print(f"Skipping hypernetwork update: ref_values size too small ({ref_h}x{ref_w}), minimum required is 64x64")
