            ...
            with torch.no_grad():
                if args.train_mode != "normal":
                    with torch.autocast("cuda", dtype=weight_dtype):
                        ...
                        print(f"[Debug Decode] sample shape: {sample.shape} (B, F, H, W, C)")
                        print(f"[Debug Decode] sample tensor on device: {sample.device}, dtype: {sample.dtype}")
                        ...
                        print(f"[Debug Decode] image sample shape: {sample.shape} (B, F, H, W, C)")
                        os.makedirs(os.path.join(args.output_dir, "sample"), exist_ok=True)
...
def parse_args():
    ...
    parser.add_argument(
        "--low_vram", action="store_true", default=False, help="Whether enable low_vram mode."
    )
    parser.add_argument(
        "--vae_tiling", action="store_true", help="Whether enable VAE tiling encoding to save memory."
    )
    parser.add_argument(
        "--train_mode",
        type=str,
...
def main():
    ...
    # [TODO] hypernetwork
    # 计算每个LiLoRA层需要的权重维度: (up_dim + down_dim) * rank
    # 当前LiLoRA配置: down_dim=200, up_dim=100, rank=1
    lora_weight_dim = (args.down_dim + args.up_dim) * 1  
    ...
    hypernetwork = hypernetwork.to(weight_dtype)
    hypernetwork.train()
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process():
                ...
                if network_ is not None:
                    ...
                    accelerator.print(f"\nSaving lora checkpoint: {safetensor_save_path}")
                    network_.save_weights(safetensor_save_path, weight_dtype, None)
                
                if hypernetwork_ is not None:
                    hyper_save_path = os.path.join(output_dir, f"hypernetwork.safetensors")
                    accelerator.print(f"\nSaving hypernetwork checkpoint: {hyper_save_path}")
                    
                    from safetensors.torch import save_file
                    hyper_state_dict = {}
                    for key, value in hypernetwork_.state_dict().items():
                        v = value.detach().clone().cpu()
                        if weight_dtype is not None:
                            v = v.to(weight_dtype)
                        hyper_state_dict[key] = v
                    save_file(hyper_state_dict, hyper_save_path, {})
                    accelerator.print(f"HyperNetwork weights saved to {hyper_save_path}")
        ...
        def load_model_hook(models, input_dir):
            ...
            if network_ is not None:
                lora_path = os.path.join(input_dir, f"lora_diffusion_pytorch_model.safetensors")
                if os.path.exists(lora_path):
                    from safetensors.torch import load_file
                    lora_state_dict = load_file(lora_path)
                    network_.load_state_dict(lora_state_dict, strict=False)
                    accelerator.print(f"LoRA weights loaded from {lora_path}")
            
            if hypernetwork_ is not None:
                hyper_path = os.path.join(input_dir, f"hypernetwork.safetensors")
                if os.path.exists(hyper_path):
                    from safetensors.torch import load_file
                    hyper_state_dict = load_file(hyper_path)
                    hypernetwork_.load_state_dict(hyper_state_dict)
    ...
    if args.gradient_checkpointing:
        if hasattr(transformer3d, 'gradient_checkpointing'):
            transformer3d.gradient_checkpointing = True
        else:
    ...
    hypernetwork_lr = args.learning_rate * 1  # HyperNetwork 使用 10 倍学习率
    hypernetwork_trainable_params = [{"params": list(filter(lambda p: p.requires_grad, hypernetwork.parameters())), "lr": hypernetwork_lr}]
    params_to_optimize = trainable_params_optim + hypernetwork_trainable_params  # 主网络在前
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            for example in examples:
                ...
                # ref_values: 保持 (F, H, W, 3) 格式，不做 permute
                new_examples["ref_values"].append(example["ref_values"])
            ...
            new_examples["ref_values"] = torch.stack([torch.from_numpy(example[:batch_video_length]) for example in new_examples["ref_values"]])
            new_examples["original_size"] = original_size

            if args.train_mode != "normal":
                B, F, C, H, W = new_examples["pixel_values"].shape
                mask = torch.zeros((B, F, 1, H, W), device=new_examples["pixel_values"].device)
                mask[:, 1:, :, :, :] = 1.0

                masked_pixel_values = new_examples["pixel_values"] * (1 - mask) + (-1) * mask

                new_examples["mask"] = mask
                new_examples["mask_pixel_values"] = masked_pixel_values
    ...
    network, optimizer, hypernetwork, train_dataloader, lr_scheduler = accelerator.prepare(
        network, optimizer, hypernetwork, train_dataloader, lr_scheduler
    )
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            from safetensors.torch import load_file
            lora_path = os.path.join(checkpoint_folder_path, "lora_diffusion_pytorch_model.safetensors")
            if os.path.exists(lora_path):
                lora_state_dict = load_file(lora_path, device=str(accelerator.device))
                m, u = accelerator.unwrap_model(network).load_state_dict(lora_state_dict, strict=False)
                accelerator.print(f"LoRA weights loaded from {lora_path} (missing: {len(m)}, unexpected: {len(u)})")
            
            hyper_path = os.path.join(checkpoint_folder_path, "hypernetwork.safetensors")
            if os.path.exists(hyper_path):
                hyper_state_dict = load_file(hyper_path, device=str(accelerator.device))
                accelerator.unwrap_model(hypernetwork).load_state_dict(hyper_state_dict)
                accelerator.print(f"HyperNetwork weights loaded from {hyper_path}")
    ...
    def save_model(checkpoint_folder, unwrapped_nw, unwrapped_hypernetwork=None):
        os.makedirs(checkpoint_folder, exist_ok=True)
        accelerator.print(f"\nsaving checkpoint to: {checkpoint_folder}")
        
        safetensor_save_path = os.path.join(checkpoint_folder, f"checkpoint-{os.path.basename(checkpoint_folder)}.safetensors")
        unwrapped_nw.save_weights(safetensor_save_path, weight_dtype, None)
        accelerator.print(f"LoRA weights saved to {safetensor_save_path}")
        
        if unwrapped_hypernetwork is not None:
            hyper_save_path = os.path.join(checkpoint_folder, "hypernetwork.safetensors")
            from safetensors.torch import save_file
            hyper_state_dict = {}
            for key, value in unwrapped_hypernetwork.state_dict().items():
                v = value.detach().clone().cpu()
                if weight_dtype is not None:
                    v = v.to(weight_dtype)
                hyper_state_dict[key] = v
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                # Handle random frame crop for target video and ref video
                ...
                # HyperNetwork: encode ref video and update LiLoRA weights (端到端训练)
                # Must be after auto_tile_batch_size and random_frame_crop to keep frame sync
                if "ref_values" in batch:
                    print("HyperNetwork is triggered!")
                    # collate_fn 已经 stack 成 tensor，格式 (B, F, H, W, 3)
                    # 需要 permute 到 (B, 3, F, H, W) 给 VAE
                    ref_pixel_values = batch["ref_values"].permute(0, 4, 1, 2, 3).contiguous()
                    ref_pixel_values = ref_pixel_values.to(weight_dtype)
                    ref_pixel_values = ref_pixel_values / 255.
                    ref_pixel_values = ref_pixel_values[:, :, :pixel_values.size(2), :, :]
                    
                    print(f"[Debug VAE Encode] ref_pixel_values shape: {ref_pixel_values.shape} (B, C, F, H, W)")
                    print(f"[Debug VAE Encode] pixel_values shape: {pixel_values.shape} (B, C, F, H, W)")
                    
                    with accelerator.autocast():
                        # VAE encode ref_video -> ref_video_features
                        with torch.no_grad():
                            if args.low_vram:
                                ref_video_features = vae.encode(ref_pixel_values.cpu()).latent_dist.sample()
                            else:
                            ...
                            ref_video_features = ref_video_features.to(accelerator.device)
                        
                        # HyperNetwork: ref_video_features -> weights
                        pred_weights, pred_weights_list = hypernetwork(ref_video_features)

                        # Get the underlying module for distributed training compatibility
                        actual_network = network.module if hasattr(network, 'module') else network

                        # Update LiLoRA weights (端到端训练，loss 会反向传播到 hypernetwork)
                        for i, (weight, lora_layer) in enumerate(zip(pred_weights_list, actual_network.unet_loras)):
                            # 调试打印：检查超网络输出尺寸和LoRA期望尺寸
                            expected_lora_size = (lora_layer.up_dim + lora_layer.down_dim) * lora_layer.rank
                            #print(f"[Debug] lora_layer {i}: up_dim={lora_layer.up_dim}, down_dim={lora_layer.down_dim}, rank={lora_layer.rank}")
                            #print(f"[Debug] expected LoRA size: {expected_lora_size}")
                            #print(f"[Debug] weight shape before view: {weight.shape}")
                            
                            # weight: [B, Wn, Wd] 或 [Wn, Wd]
                            # 展平到 LoRA 期望的格式 [B, Wn*Wd] 或 [Wn*Wd]
                            if weight.dim() == 3:
                            ...
                            #print(f"[Debug] weight shape after view: {weight.shape}")
                ...
                with torch.no_grad():
                    ...
                    # VAE decode debug verification (every 100 steps)
                    if global_step % 1 == 0 and accelerator.is_main_process:
                        original_size = batch.get("original_size", None)
                        print(f"\n[Debug VAE Decode Verification @ Step {global_step}]")
                        if original_size is not None:
                            print(f"[Debug VAE] Original video size (before resize): {original_size}")
                        print(f"[Debug VAE] Resized pixel_values shape: {pixel_values.shape} (B, F, C, H, W)")
                        
                        # Move latents to GPU if needed for decode
                        latents_for_decode = latents.to(accelerator.device)
                        if vae.use_tiling:
                            latents_for_decode = latents_for_decode[:, :, :pixel_values.shape[1], :, :]
                        
                        with torch.no_grad():
                            decoded = vae.decode(latents_for_decode).sample
                        
                        print(f"[Debug VAE] Latent shape: {latents_for_decode.shape} (B, C, F, H, W)")
                        print(f"[Debug VAE] Decoded shape: {decoded.shape} (B, C, F, H, W)")
                        
                        # if original_size is not None:
                        #     expected_H, expected_W = original_size
                        #     actual_H, actual_W = decoded.shape[-2], decoded.shape[-1]
                        #     print(f"\n[Debug Size Verification]")
                        #     print(f"[Debug] Comparing: Original video ({expected_H}, {expected_W}) vs Decoded ({actual_H}, {actual_W})")
                            
                        #     if actual_H == expected_H and actual_W == expected_W:
                        #         print(f"[Debug] ✓ Size MATCH! VAE decode correctly restores original size")
                        #     else:
                        #         print(f"[Debug] ✗ Size MISMATCH!")
                        #         print(f"[Debug]   Expected: ({expected_H}, {expected_W})")
                        #         print(f"[Debug]   Got:      ({actual_H}, {actual_W})")
                        #         print(f"[Debug]   Difference: H={actual_H-expected_H} ({((actual_H-expected_H)/expected_H*100):.1f}%), W={actual_W-expected_W} ({((actual_W-expected_W)/expected_W*100):.1f}%)")
                        ...
                        #         print(f"[Debug] Resized pixel_values shape: {pixel_values.shape}")
                        #         print(f"[Debug] This means: Resized ({pixel_values.shape[-2]}, {pixel_values.shape[-1]}) -> Latent -> Decoded ({actual_H}, {actual_W})")
                        # else:
                        #     orig_H, orig_W = pixel_values.shape[-2], pixel_values.shape[-1]
                        #     dec_H, dec_W = decoded.shape[-2], decoded.shape[-1]
                            
                        #     if orig_H == dec_H and orig_W == dec_W:
                        #         print(f"[Debug VAE] ✓ Size matches! Resized: ({orig_H}, {orig_W}), Decoded: ({dec_H}, {dec_W})")
                        #     else:
                        #         print(f"[Debug VAE] ✗ Size mismatch! Resized: ({orig_H}, {orig_W}), Decoded: ({dec_H}, {dec_W})")
                        # print("-" * 80)
                ...
                def _batch_decode_vae(latents, original_size=None):
                    """
                    VAE decode with debug prints to verify output size matches input.
                    """
                    print(f"[Debug Decode] Latent input shape: {latents.shape} (B, C, F, H, W)")
                    if original_size is not None:
                        print(f"[Debug Decode] Original pixel_values size: {original_size} (H, W)")
                    
                    decoded = vae.decode(latents).sample
                    print(f"[Debug Decode] Decoded output shape: {decoded.shape} (B, C, F, H, W)")
                    
                    if original_size is not None:
                        expected_H, expected_W = original_size[-2], original_size[-1]
                        actual_H, actual_W = decoded.shape[-2], decoded.shape[-1]
                        if actual_H == expected_H and actual_W == expected_W:
                            print(f"[Debug Decode] ✓ Size matches! Expected: ({expected_H}, {expected_W}), Got: ({actual_H}, {actual_W})")
                        else:
                            print(f"[Debug Decode] ✗ Size mismatch! Expected: ({expected_H}, {expected_W}), Got: ({actual_H}, {actual_W})")
                ...
                #     hypernetwork_params_ids = set(id(p) for p in hypernetwork.parameters())
                #     hp_lr = None
                #     for group in optimizer.param_groups:
                #         if any(id(p) in hypernetwork_params_ids for p in group["params"]):
                #             hp_lr = group["lr"]
                #             break
                    
                #     first_layer_weight = hypernetwork.img_weight_generator.feature_proj.weight
                    
                #     # 检查权重是否更新
                #     if hasattr(hypernetwork, '_last_weight_val'):
                #         weight_diff = (first_layer_weight - hypernetwork._last_weight_val).norm().item()
                #         update_status = "✅ 更新" if weight_diff > 1e-8 else "❌ 未更新"
                #     else:
                #         weight_diff = 0.0
                #         update_status = "首次"
                    
                #     print(f"[Step {global_step}] loss={loss.item():.4f} | HN_grad={total_grad_norm:.8f} | HN_lr={hp_lr} | weight_diff={weight_diff:.8f} | {update_status}")
                    
                #     hypernetwork._last_weight_val = first_layer_weight.data.clone()
                # # ===== End Debug =====
                ...
                if global_step % args.checkpointing_steps == 0:
                    if args.use_deepspeed or accelerator.is_main_process:
                        ...
                        if not args.save_state:
                            checkpoint_folder = os.path.join(args.output_dir, f"checkpoint-{global_step}")
                            save_model(checkpoint_folder, accelerator.unwrap_model(network), accelerator.unwrap_model(hypernetwork))
