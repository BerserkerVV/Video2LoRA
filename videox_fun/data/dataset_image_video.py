        ...
        with VideoReader_contextmanager(video_dir, num_threads=2) as video_reader:
            if for_ref:
                ...
                if min_sample_n_frames == 0:
                    raise ValueError(f"No frames in video: {video_dir}")
            ...
            try:
                ...
                for frame in pixel_values:
                    resized_frames.append(resize_frame(frame, self.larger_side_of_image_and_video))
                pixel_values = np.array(resized_frames)
            except FunctionTimedOut:
                raise ValueError(f"Read video timeout: {video_dir}")
            except Exception as e:
                raise ValueError(f"Failed to extract frames from video: {video_dir}. Error: {e}")
            finally:
                del video_reader
    ...
    def __getitem__(self, idx):
        while True:
            try:
                ...
                ref_pixel_values, _, _, _ = self.get_batch(ref_idx, for_ref=True)

                sample = {
                    "pixel_values": pixel_values,
                    "ref_values": ref_pixel_values,
                    "text": text,
                    "data_type": data_type,
                    "idx": idx
                }
