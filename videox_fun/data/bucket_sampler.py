    ...
    def __iter__(self):
        for idx in self.sampler:
            try:
                ...
                print(f"Dataset item: {self.dataset[idx]}")
                print("This item is error, please check it.")
                continue
            # find the closest aspect ratio
            closest_ratio = min(self.aspect_ratios.keys(), key=lambda r: abs(float(r) - ratio))
            if closest_ratio not in self.current_available_bucket_keys:
...
class AspectRatioBatchImageVideoSampler(BatchSampler):
    ...
    def __init__(self,
        ...
        self.current_available_bucket_keys = list(aspect_ratios.keys())
        self.bucket = {ratio: [] for ratio in aspect_ratios}
    ...
    def __iter__(self):
        for idx in self.sampler:
            try:
                ...
                if width is None or height is None:
                    video_id, name = video_dict.get('file_path', video_dict.get('video_path', '')), video_dict.get('text', video_dict.get('video_caption', ''))
                    if self.train_folder is None:
                        video_dir = video_id
                    else:
            ...
            if closest_ratio not in self.current_available_bucket_keys:
                continue
            bucket = self.bucket[closest_ratio]
            bucket.append(idx)
